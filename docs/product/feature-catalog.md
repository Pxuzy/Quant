...
| 层级 | 入口 | 用途 |
| --- | --- | --- |
| 工作台层 | 总控台、股票池、股票详情、新闻汇总、数值数据 | 面向每天查看股票、行情、新闻和数值的研究流程 |
| 控制面 | 数据源管理、同步调度、数据库管理 | 面向数据来源、同步、质量、批次、血缘和缺口修复 |
...
| 产品层级 | 工作台层 |
| 前端路由 | `/news` |
| 页面文件 | `frontend/src/pages/news/NewsPage.tsx` |
| 当前能力 | 新闻类能力入口和辅助页面 |
...
| 页面文件 | `frontend/src/pages/data-system/numeric-summary/NumericSummaryPage.tsx` |
| 当前能力 | 数值数据汇总入口和辅助页面 |
...
| 数据对象 | `data_sources` |
| 支持来源 | 当前 `akshare`, `baostock`, `stock_sdk`；`adata`, `tushare` 待实现 |
