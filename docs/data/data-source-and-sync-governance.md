...
当前 API 正式管线没有强制保存 provider 原始响应。`scripts/ops/fetch_daily_bars.py` 与 `scripts/ops/merge_raw_to_silver.py` 产生的 `storage/raw` 是旁路运维数据，不自动等同于 `sync_tasks`、`ingest_batches`、quality 和 lineage。
