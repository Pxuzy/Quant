...
它不是大型数据湖平台，也不是一开始就追求完整量化交易系统。当前阶段先把 A 股股票池、日线、交易日历、数据源、同步任务、批次、质量和血缘跑成闭环，再把这些能力组织成更顺手的研究入口。
...
| 层级 | 目标 |
| --- | --- |
| 工作台层 | 让用户每天能看关注股票、行情数值、新闻事件和单股详情 |
| 控制面 | 让用户知道数据从哪里来、是否新鲜、缺什么、怎么补、能否追溯 |

数据库管理不是最终产品本身，而是研究台可信度的控制面。后续页面设计和功能优先级都按这个口径收敛。
...
| 同步执行 | 轻量 worker |
| 数据源 | AKShare、BaoStock、Stock SDK；AData/Tushare 尚未注册 |
...
| 层级 | 入口 | 作用 |
| --- | --- | --- |
| 工作台层 | 总控台 | 展示研究入口、关键数据状态、最近任务和快捷操作 |
| 工作台层 | 股票池 | A 股列表、搜索筛选、单股详情和日线数据 |
| 工作台层 | 新闻汇总 | 当前是新闻能力入口，后续接入真实新闻和股票关联 |
| 工作台层 | 数值数据 | 当前是数值数据入口，后续聚合常用行情和指标 |
| 控制面 | 数据源管理 | 数据源启用状态、优先级、能力、健康检查、真实取样 |
| 控制面 | 同步调度 | 手动同步、定时配置、任务记录、日志、入库批次 |
| 控制面 | 数据库管理 | 数据库状态、数据湖状态、数据集、血缘、质量、水位线 |
